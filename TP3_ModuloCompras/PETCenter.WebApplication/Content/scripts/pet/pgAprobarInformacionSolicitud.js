...
function SetVerSolicitudId(primarykey) {
    ...
    $('#divSeccionCodigo').append(
        ...
        "<div class=\"col-xs-6 col-sm-6 col-md-3\">" +
        "    <div class=\"input-group input-group-sm\">" +
        "        <span class=\"input-group-addon\" style=\"width:130px\">Saldo Inicial : S/. </span>" +
        "        <input id=\"txtSaldoInicialModal\" class=\"form-control input-lg text-uppercase text-uppercase\" style=\"width: 200px\" />" +
        "    </div>" +
        "</div>" +
        "<div class=\"col-xs-6 col-sm-6 col-md-3\">" +
        "    <div class=\"input-group input-group-sm\">" +
        "        <span class=\"input-group-addon\" style=\"width:130px\">Saldo Final : S/.</span>" +
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.messageType == "ERR")
                ...
                $("#txtSaldoInicialModal").val(response.rows[0].Presupuesto.Monto);
                ...
                VerItemsSolicitud('GetItemSolicitudRecurso_A', primarykey, 0, 0,0,0);
                $("#txtSaldoFinalModal").val(response.rows[0].Presupuesto.Monto - $("#txtTotalModal").val());

                    if ($("#txtEstadoTempModal").val() == 'APROBADO'){

                    if ($("#txtSaldoFinalModal").val() < 0) {
        ...
        success: function (response) {
            if (response.messageType == "ERR")
                ...
                if (response.rows[0].Estado == "EMI") {
                    ...
                    GetItemsSolicitud('GetItemSolicitudRecurso_A', primarykey, 0, 0, 0, 0);
                }
                else
                {
                ...
                read: function (options) {
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            for (var i = 0; i < response.length; i++) { console.log('NO FALLAR')}
...
function VerItemsSolicitud(accion, idsolicitudrecurso, idpresentacionrecurso, cantidad) {
    ...
    $("#gridItemsSolicitudModal").kendoGrid({
                ...
                read: function (options) {
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            console.log('RESPONDE 6');
                            console.log('RESPONDE 7 ====>' + response.rows.length);
        ...
        rowTemplate: "<tr>" +
            ...
            "<td>#: presentacionrecurso.stock #</td>" +
            "<td>S/.#: precioreferencial #</td>" +
            "<td>S/.#: total #</td>" +
            "</tr>",
        altRowTemplate: "<tr>" +
